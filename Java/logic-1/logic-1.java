...
public boolean squirrelPlay(int temp, boolean isSummer) {
    if (temp >= 60) {
        return (temp <= 90 || isSummer && temp <= 100);
    }
    return false;
}
...
public String alarmClock(int day, boolean vacation) {
    String result;
    Boolean weekdays = day >= 1 && day <= 5;
    if (vacation) {
        result = weekdays ? "10:00" : "off";
    } else {
        result = weekdays ? "7:00" : "10:00";
    }
    return result;
}
...
public boolean love6(int a, int b) { 
    ...
    if ((a == 6 || b == 6) || (a + b == 6 || diff == 6)) return true;
    return false;
}
...
public boolean old35(int n) {
  ...
  return (n % 15 == 0) ? false : ((n % 3 == 0 || n % 5 == 0) ? true : false);
}
...
public boolean less20(int n) {
    return  (n % 20 >= 18 && n % 20 <= 20) ? true : false;
}
...
public int teaParty(int tea, int candy) {
    int result = 0;
    if (tea >= 5 && candy >= 5) {
        result =  (tea >= (candy * 2) || candy >= (tea * 2)) ? 2 : 1;
    }
    return result;
}
...
public boolean lastDigit(int a, int b, int c) {
    ...
    int b1 = b % 5;
    int c1 = c % 5;
    return (a1 == c1 || a1 == b1 || b1 == c1) ? true : false;
}
...
public int sumLimit(int a, int b) {
    int sum = a + b;
    int aval = String.valueOf(a).length();
    int sumVal = String.valueOf(sum).length();
    return sumVal > aval ?  a : sum;
}